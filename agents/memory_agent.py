...
# Fonction pour appeler le modèle LLaMA2 via Ollama
...
# Récupérer l'historique d'un sujet donné
def get_topic_history(topic: str):
    result = db.search(Topic.name == topic)
    return result[0]["messages"] if result else []

# Mettre à jour la mémoire avec une nouvelle interaction
def update_memory(topic: str, user_msg: str, ai_msg: str):
    if db.contains(Topic.name == topic):
        db.update(
            lambda t: t["messages"].append({"user": user_msg, "ai": ai_msg}),
            Topic.name == topic
        )
    else:
        db.insert({
            "name": topic,
            "messages": [{"user": user_msg, "ai": ai_msg}]
        })

# Fonction principale de l'agent mémoire
